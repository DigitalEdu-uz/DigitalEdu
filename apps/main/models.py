...
class Category(db.Model):
    """Категория"""
    image = db.FileField("Фото", upload_to='contents/image/%Y_%m/')
...
class Course(db.Model):
    ...
    intro = db.TextField("Введение")
    price = db.PositiveIntegerField("Цена")
    content = db.FileField("Контент", upload_to='contents/%Y_%m/')
    url = db.SlugField("Ссылка", max_length=60, unique=True)
    ...
    created_at = db.DateTimeField("Дата", default=timezone.now)
